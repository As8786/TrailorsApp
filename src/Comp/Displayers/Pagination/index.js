...
const Pagination = ({ collectionLength, currentPage, handlePagination, applyOpacity }) => {
    ...
    return <div className="pagination-container">
        <button id="btn-text" onClick={() => {
            applyOpacity()
            onPaginationClick(null, "Prev")
        }}> Prev </button>

        <button id="btn-text" onClick={() => {
            applyOpacity()
            onPaginationClick(null, "First")
        }}> First </button>

        {[...new Array(pagesNumber)].map((el, i) => <button key={i} id="btn-number" className={i + 1 === actualPage ? "active" : ""} onClick={() => {
            applyOpacity()
        ...
        <button id="btn-text" onClick={() => {
            applyOpacity()
            onPaginationClick(null, "Last")
        }}> Last</button>

        <button id="btn-text" onClick={() => {
            applyOpacity()
            onPaginationClick(null, "Next")
        }}> Next </button >
    </div >
}
